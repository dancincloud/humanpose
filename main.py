        ...
        while True:
            ...
            if hasattr(self.server, 'frametosend'):
                image = Image.fromarray(cv2.cvtColor(
                    self.server.frametosend, cv2.COLOR_BGR2RGB))
                stream_file = BytesIO()
                ...
                self.send_header('Content-length',
                                 str(stream_file.getbuffer().nbytes))
                self.end_headers()
...
server_TCP = socketserver.TCPServer((HTTP_LOCAL_HOST, 8070), TCPServerRequest)
...
server_HTTP = ThreadedHTTPServer(
    (HTTP_LOCAL_HOST, HTTP_SERVER_PORT), VideoStreamHandler)
    ...
    try:
        while should_run():
            ...
            if debug:
                if keypoints_list is not None and detected_keypoints is not None and personwiseKeypoints is not None:
                    for i in range(18):
                        for j in range(len(detected_keypoints[i])):
                            cv2.circle(
                                debug_frame, detected_keypoints[i][j][0:2], 5, colors[i], -1, cv2.LINE_AA)
                    for i in range(17):
                        for n in range(len(personwiseKeypoints)):
                            ...
                            cv2.line(
                                debug_frame, (B[0], A[0]), (B[1], A[1]), colors[i], 3, cv2.LINE_AA)
                ...
                server_HTTP.frametosend = debug_frame
